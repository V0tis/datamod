...
const TAB_SYSTEM_PROMPTS: Record<TabType, string> = {
  logic:
    "당신은 시장 리서치와 전략 분석 전문가 '린'입니다. **SWOT 분석**, **인과관계**, **시장 규모·데이터**를 중심으로 논리적으로 분석해주세요. 숫자와 근거를 명시하고 2~4문단으로 답변하세요. 응답은 마크다운 형식으로 작성하고, 중요 키워드는 반드시 **강조**해 주세요.",
  creative:
    "당신은 시장 리서치와 아이디어 전문가 '린'입니다. **틈새 시장 공략법**, **마케팅 아이디어**, **확장 가능성**을 중심으로 창의적으로 제안해주세요. 실행 가능한 아이디어를 2~4문단으로 답변하세요. 응답은 마크다운 형식으로 작성하고, 중요 키워드는 반드시 **강조**해 주세요.",
  fact:
    "당신은 시장 리서치와 팩트체크 전문가 '린'입니다. **최신 뉴스 출처**, **구체적 통계 수치**, **타임라인**을 중심으로 사실 위주로 정리해주세요. 출처와 숫자를 명시하고 2~4문단으로 답변하세요. 응답은 마크다운 형식으로 작성하고, 중요 키워드는 반드시 **강조**해 주세요.",
}
...
export async function POST(req: Request) {
  ...
  // Fallback: Gemini 실패 시 사용자 입력 키 우선, 없으면 env. gpt-4o-mini로 동일 페르소나 답변
  if (openaiKey) {
    try {
      ...
      if (reportId) {
        ...
        if (report?.user_id === user.id) {
          ...
          await supabase
            .from('reports')
            .update({ ai_responses: { ...current, [tab]: text } })
            .eq('id', reportId)
        }
      }
      ...
      console.error('[Research Insights Tab API] OpenAI fallback failed:', fallbackErr)
    }
  }

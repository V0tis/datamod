...
export interface ResultTimelineSectionProps {
  ...
  embedded?: boolean
  /** AI 우선 모델 (task.provider fallback). priority: step > run > setting */
  aiPrimaryModel?: 'gemini' | 'groq'
  /** reportId - 변경 시 타임라인 상태 초기화 */
...
export function ResultTimelineSection({
  ...
  return (
    <div
      id="section-timeline"
      className={className}
      style={effectiveMaxHeight ? { maxHeight: effectiveMaxHeight, overflowY: 'auto' } : undefined}
    >

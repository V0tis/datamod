...
export interface StreamingInsightTextProps {
  /** Bullet items to reveal one-by-one */
  items: Array<{ label: string; value: string }>
  /** When true, animate reveal with streaming indicator */
  streaming?: boolean
  /** Delay (ms) between revealing each item */
  revealDelayMs?: number
  /** Callback when all items have been revealed */
  onComplete?: () => void
  /** Skip animation and show all immediately */
  skipAnimation?: boolean
  className?: string
}

/**
 * Reveals insight bullets progressively with a streaming/typing effect.
 * When streaming=true and items exist, reveals them one-by-one with a blinking cursor.
 */
export function StreamingInsightText({
  items,
  streaming = false,
  revealDelayMs = 350,
  onComplete,
  ...
  useEffect(() => {
    ...
    if (itemsKey !== prevItemsKey.current) {
  ...
  }, [revealedCount, items.length, streaming, revealDelayMs, skipAnimation, onComplete])

  const isStreaming = streaming && revealedCount < items.length
  const showCursor = streaming && revealedCount > 0 && (isStreaming || items.length === revealedCount)
  ...
  return (
    <ul className={cn('space-y-3', className)}>
      {items.slice(0, revealedCount).map((item, i) => (
        <li key={`${item.label}-${i}`} className="flex gap-3 animate-in fade-in slide-in-from-bottom-1 duration-200">

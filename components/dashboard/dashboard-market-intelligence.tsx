...
const cardClass =
  'rounded-[12px] border border-[#E5E9F2] bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)] md:px-6'

export function DashboardScatterMatrixCard({
  scatterData,
  scatterLoading,
  countryCode,
  className,
  ...
  return (
    <div className={cn(cardClass, 'h-full min-h-0 px-5 py-5', className)} style={{ padding: '20px 24px' }}>
      <h2 className={sectionTitle}>기회·리스크 매트릭스</h2>
      <p className="mt-1 text-[12px] text-[#6B7280]">각 키워드를 기회·리스크 좌표로 배치했습니다. 버블 크기는 트렌드 강도입니다.</p>
      <div className="mt-4">
...
export function DashboardTrendGrowthCard({
  trendItems,
  trendsLoading,
  className,
}: {
  ...
  return (
    <div className={cn(cardClass, 'px-5 py-5', className)} style={{ padding: '20px 24px' }}>
      <h2 className={sectionTitle}>트렌드 성장 추이</h2>
      <p className="mt-1 text-[12px] text-[#6B7280]">상위 키워드 관심도 추정</p>
...
export function DashboardTopOpportunitiesCard({
  topOpportunities,
  className,
}: {
  ...
  return (
    <div className={cn(cardClass, 'px-5 py-5', className)} style={{ padding: '20px 24px' }}>
      ...
      <p className="mt-1 text-[12px] text-[#6B7280]">추정 기회 점수 상위 5개</p>
      <ol className="mt-4 space-y-3">
        {topOpportunities.length === 0 ? (
          <li className="text-sm text-[#6B7280]">비즈니스 관련 트렌드가 쌓이면 목록이 표시됩니다</li>
        ) : (
          topOpportunities.map((row, idx) => (
            <li
              key={row.keyword}
              className="flex items-center justify-between gap-3 border-b border-[#F3F4F6] pb-3 last:border-0"
            >
              <div className="flex min-w-0 items-center gap-3">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-[13px] font-bold tabular-nums text-[#374151]">
                  {idx + 1}
                </span>
                <div className="min-w-0">
                  <p className="truncate text-[13px] font-semibold text-[#111827]">{row.keyword}</p>
                  <p className="truncate text-[11px] text-[#6B7280]">{row.sub}</p>
                </div>
              </div>
              <span className="shrink-0 text-[22px] font-bold tabular-nums text-[#111827] [font-variant-numeric:tabular-nums]">
                {row.score}
              </span>
            </li>
          ))

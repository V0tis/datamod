...
export function DashboardChartsBlock({
  ...
  const layoutClass = variant === 'stack' ? 'flex flex-col gap-8' : 'grid grid-cols-1 gap-8 lg:grid-cols-3'

  if (loading) {
    return (
      <div className={layoutClass}>
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-[200px] animate-pulse rounded-lg bg-slate-100 dark:bg-zinc-900" />
        ))}
      </div>
    )
  }

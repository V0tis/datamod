 ...
 */
export const PASS1_SYSTEM = `뉴스 기반 시장 분석. JSON만. 한국어. 결정적 문장. 섹션당 최대 3문장.
...
export function buildPass1Prompt(keyword: string, newsTitles: string[]): string {
  ...
  return `${block}"${keyword}" 분석. summary(1문장), temperature(0-100), insights(정확히 3개). JSON만.`
}

export const PASS2_SYSTEM = `뉴스 기반 PM 액션. JSON만. 한국어. 결정적 문장.
...
export function buildPass2Prompt(keyword: string, newsTitles: string[], pass1Summary: string): string {
  const block = newsTitles.length ? `뉴스:\n${newsTitles.slice(0, 12).map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\n` : ''
  return `${block}1차 요약: ${pass1Summary}\n\n"${keyword}" 상세. facts 3~5, hypotheses 0~3, inferences 2~4. actions 2~4. signals 각 1~3. JSON만.`
}
